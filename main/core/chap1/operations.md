...
## 算術演算子
算術演算子は数値をオペランドに取り、演算の結果をNumber値で一つ返します。
...
## 同値演算子
同値演算子(等価演算子)はオペランドを比較し、その結果のBoolean値を返します。オペランドには数値、文字列、論理値、オブジェクトを使用でき、演算子は`==` `!=` `===` `!==`の4種類があります。他の言語には見られない`===` `!==`の役割については後述します。
...
## 論理演算子
JavaScriptの論理演算子には`&&` `||` `!`があります。基本的にはBoolean値を返します。
...
## 暗黙の型変換
JavaScriptでは異なる型の値同士で演算するときに、型変換を行います。
...
型変換が行われないので、String型の`'10'`とNumber型の`10`は別のものと判断され、`false`が返ってきます。
